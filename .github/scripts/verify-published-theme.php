...
/**
 * Asserts registration after installing a published theme in a fresh site.
 */
$theme = \Drupal::service('theme_handler')->listInfo()['emulsify'] ?? NULL;
if ($theme === NULL || !$theme->status) {
  throw new RuntimeException('Published Emulsify theme was not enabled.');
}
if (\Drupal::config('system.theme')->get('default') !== 'emulsify') {
  throw new RuntimeException('Published Emulsify theme was not set as default.');
}
...
if ($breakpoints !== $expected) {
  throw new RuntimeException('Published Emulsify breakpoint registration differs from its public contract.');
}
