...
In this fixture, Drupal 11.4's direct `core/scripts/drupal` entrypoint failed to locate the Composer autoloader in the recommended-project layout. The [Drupal 11.4.0 announcement](https://www.drupal.org/blog/drupal-11-4-0) introduces `vendor/bin/dr` as experimental; it does not deprecate the direct entrypoint. The smoke runner uses the installed `vendor/bin/dr` proxy when available, retaining the direct entrypoint for earlier supported cores.
